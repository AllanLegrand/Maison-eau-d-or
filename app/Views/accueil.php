<main class="container my-5">
		...
		<div class="carousel-inner">
			...
			<div class="carousel-item active">
				...
				<div class="carousel-caption d-none d-md-block">
					...
					<p class="lead">Découvrez la parfumerie Maison Eau d'Or, votre destination pour des parfums uniques
						et élégants.</p>
				</div>
			</div>
	...
	<section class="my-5">
		<div class="text-center bordergold">
			...
			<h2 class="section-title active" id="bestsellersTitle" onclick="toggleContent('bestsellers')">Bestsellers
			</h2>
		...
		<div id="bestsellersContent" class="carousel slide mt-4" data-bs-ride="carousel">
					...
					<div class="carousel-item <?= $index === 0 ? 'active' : '' ?>">
						<div class="carousel-container d-flex justify-content-center">
							<?php foreach ($row as $produitBestseller): ?>
								<div class="product-item col-12 col-sm-6 col-md-4 col-xl-3 mx-3"
									onclick="openModal(<?= $produitBestseller['id_prod'] ?>)">
									<?php
									$imagePath = base_url('assets/img/' . $produitBestseller['img_path']);
									$defaultImage = base_url('assets/img/default.png');
									?>
									<img src="<?= file_exists('./assets/img/' . $produitBestseller['img_path']) ? $imagePath : $defaultImage ?>"
										alt="<?= esc($produitBestseller['nom']) ?>" class="product-img">
			...
			<button class="carousel-control-prev custom-prev" type="button" data-bs-target="#bestsellersContent"
				data-bs-slide="prev">
				<img src="/assets/img/back.png" alt="Previous" class="custom-arrow">
				<span class="visually-hidden">Previous</span>
			</button>
			<button class="carousel-control-next custom-next" type="button" data-bs-target="#bestsellersContent"
				data-bs-slide="next">
		...
		<div id="coffretsContent" class="carousel slide mt-4 d-none" data-bs-ride="carousel">
					...
					<div class="carousel-item <?= $index === 0 ? 'active' : '' ?>">
						<div class="carousel-container d-flex justify-content-center">
							<?php foreach ($row as $produitCoffret): ?>
								<div class="product-item col-12 col-sm-6 col-md-4 col-xl-3 mx-3"
									onclick="openModal(<?= $produitCoffret['id_prod'] ?>)">
									<?php
									$imagePath = base_url('assets/img/' . $produitCoffret['img_path']);
									$defaultImage = base_url('assets/img/default.png');
									?>
									<img src="<?= file_exists('./assets/img/' . $produitCoffret['img_path']) ? $imagePath : $defaultImage ?>"
										alt="<?= esc($produitCoffret['nom']) ?>" class="product-img">
			...
			<!-- Contrôles pour le carrousel avec flèches -->
			<button class="carousel-control-prev custom-prev" type="button" data-bs-target="#coffretsContent"
				data-bs-slide="prev">
				<img src="/assets/img/back.png" alt="Previous" class="custom-arrow">
				<span class="visually-hidden">Previous</span>
			</button>
			<button class="carousel-control-next custom-next" type="button" data-bs-target="#coffretsContent"
				data-bs-slide="next">
				<img src="/assets/img/next.png" alt="Next" class="custom-arrow">
				<span class="visually-hidden">Next</span>
			</button>
	...
	<section class="my-5">
		...
		<div class="d-flex flex-wrap justify-content-center align-items-center mt-4">
			<?php if ($produitVedette): ?>
				...
				$defaultImage = base_url('assets/img/default.png');
				?>
				<img src="<?= file_exists('./assets/img/' . $produitVedette['img_path']) ? $imagePath : $defaultImage ?>"
					alt="<?= esc($produitVedette['nom']) ?>" class="img-fluid">
				<div class="col-md-6">
					...
					<div class="c-flex">
						...
						<button class="btn btn-dark btn-img button-margin"
							onclick="addToCart2(<?= $produitVedette['id_prod'] ?>)">Ajouter au panier <img
								src="/assets/img/ajouter-panier.svg" class="img-btn"></button>
					</div>
				</div>
			<?php else: ?>
